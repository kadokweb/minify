...
namespace KadokWeb\Minify\Test;
...
class JSTest extends TestCase
{
    protected function mockMinifier()
    {
        ...
        return $this->getMockBuilder('\KadokWeb\Minify\JS')
    ...
    {
        $this->expectException('KadokWeb\Minify\Exceptions\IOException');
...
throw new Error('.editManager.js: jQuery is required and must be loaded first')",
        ...
        // https://github.com/kadokweb/27
        $tests[] = array(
            '$.expr[":"]',
            '$.expr[":"]',
        );

        // https://github.com/kadokweb/31
        $tests[] = array(
            "$(_this).attr('src',this.src).trigger('adapt',['loader'])",
            "$(_this).attr('src',this.src).trigger('adapt',['loader'])",
        );

        // https://github.com/kadokweb/33
        $tests[] = array(
            '$.fn.alert             = Plugin
...
$.fn.alert.Constructor=Alert',
        ...
        // https://github.com/kadokweb/34
        $tests[] = array(
            'a.replace("\\\\","");hi="This   is   a   string"',
            'a.replace("\\\\","");hi="This   is   a   string"',
        );

        // https://github.com/kadokweb/35
        $tests[] = array(
            array(
        ...
        // https://github.com/kadokweb/37
        $tests[] = array(
            'function () { ;;;;;;;; }',
            'function(){}',
        );

        // https://github.com/kadokweb/40
        $tests[] = array(
            'for(v=1,_=b;;){}',
            'for(v=1,_=b;;){}',
        );

        // https://github.com/kadokweb/41
        $tests[] = array(
            "conf.zoomHoverIcons['default']",
            "conf.zoomHoverIcons['default']",
        );

        // https://github.com/kadokweb/42
        $tests[] = array(
            'for(i=1;i<2;i++);',
            'for(i=1;i<2;i++);',
        );
        ...
        // https://github.com/kadokweb/43
        $tests[] = array(
            '{"key":"3","key2":"value","key3":"3"}',
            '{"key":"3","key2":"value","key3":"3"}',
        );

        // https://github.com/kadokweb/44
        $tests[] = array(
            'return ["x"]',
            'return["x"]',
        );

        // https://github.com/kadokweb/50
        $tests[] = array(
...
BUG
    ...
    try {
        ...
        // https://github.com/kadokweb/89
        $tests[] = array(
            'for(;;ja||(ja=true)){}',
            'for(;;ja||(ja=!0)){}',
        );

        // https://github.com/kadokweb/91
        $tests[] = array(
            'if(true){if(true)console.log("test")else;}',
            'if(!0){if(!0)console.log("test")}',
        );

        // https://github.com/kadokweb/99
        $tests[] = array(
            '"object";"object2";"0";"1"',
            '"object";"object2";"0";"1"',
        );

        // https://github.com/kadokweb/102
        $tests[] = array(
            'var pb = {};',
            'var pb={}',
        );
    ...
    function otherFuncName() {
        ...
        // https://github.com/kadokweb/128
        $tests[] = array(
            'angle = (i - 3) * (Math.PI * 2) / 12; // THE ANGLE TO MARK.',
            'angle=(i-3)*(Math.PI*2)/12',
        );

        // https://github.com/kadokweb/124
        $tests[] = array(
...
return el}',
        ...
        // https://github.com/kadokweb/163
        $tests[] = array(
            'q = d / 4 / b.width()',
            'q=d/4/b.width()',
        );

        // https://github.com/kadokweb/182
        $tests[] = array(
            'label = input.val().replace(/\\\\/g, \'/\').replace(/.*\//, \'\');',
            'label=input.val().replace(/\\\\/g,\'/\').replace(/.*\//,\'\')',
        );

        // https://github.com/kadokweb/178
        $tests[] = array(
...
a = \'b\';',
        ...
        // https://github.com/kadokweb/209
        $tests[] = array(
            'var my_regexes = [/[a-z]{3}\//g, \'a string\', 1];',
            'var my_regexes=[/[a-z]{3}\//g,\'a string\',1]',
        );

        // https://github.com/kadokweb/211
        $tests[] = array(
            'if (last){
  for(i=1;i<3;i++);
} else if (first){
